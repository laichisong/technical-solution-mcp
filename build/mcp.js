...
exports.TechnicalSolutionServer = exports.TEMPLATE_STRUCTURE_TS = void 0;
...
const INITIAL_TEMPLATE_JS_OBJECT = {
    "1_overview": {
        title: "1. 概述",
        content: "",
        subsections: {
...
exports.TEMPLATE_STRUCTURE_TS = (0, immutable_1.Map)(INITIAL_TEMPLATE_JS_OBJECT); // 直接用 Immutable.Map() 构造函数
class TechnicalSolutionServer {
    constructor(initialTemplate = exports.TEMPLATE_STRUCTURE_TS) {
        this.pathSeparator = '.';
     ...
     */
    parseGetContextParams(paramsInput) {
        ...
        const params = paramsInput; // 类型断言，后续会校验
        const parsed = { sectionPath: null }; // 设置默认值
        if (params.hasOwnProperty('sectionPath')) {
            if (params.sectionPath === null || typeof params.sectionPath === 'string') {
                parsed.sectionPath = params.sectionPath;
            }
            else {
     ...
     */
    getContext(paramsInput) {
        let params = this.parseGetContextParams(paramsInput);
        ...
        if (typeof data === 'object' && data !== null && typeof data.toJS === 'function') {
            return data.toJS();
        }
        return data;
    }
     ...
     */
    parseUpdateContextParams(paramsInput) {
        if (typeof paramsInput !== 'object' || paramsInput === null) {
        ...
        return {
            path: params.path,
            content: params.content, // 保留 any 类型
            mode: parsedMode
        };
    }
    /**
     ...
     */
    updateContext(paramsInput) {
        let params = this.parseUpdateContextParams(paramsInput);
     ...
     */
    getContextAsString(sectionPathPrefix) {
        let rootNodeToFormat = this.context;
        let initialPathPartsForDisplay = [];
        ...
        const outputLines = [];
        const _recursiveFormat = (node, // Can be a Map or a primitive (e.g. content string)
        currentPathKeyParts, // Actual keys used for getIn
        indentLevel = 0) => {
        ...
        if (sectionPathPrefix) { // Formatting a sub-part
            ...
            if (title !== undefined) { // It's likely a section node
                ...
                rootNodeToFormat.forEach((node, key) => {
     ...
     */
    generateDocument() {
        const mdOutput = ["# 技术方案文档\n"];
        const _recursiveGenerateMd = (node, level = 1) => {
        ...
        this.context.forEach((node) => {
            _recursiveGenerateMd(node, 1);
        });
        return mdOutput.join("\n");
    }
}
exports.TechnicalSolutionServer = TechnicalSolutionServer;
/**
 * 将整个上下文或指定部分转换为字符串，供Agent的LLM使用。
 * @param sectionPathPrefix Optional. If provided, only formats this part of the context.
 * @returns A string representation of the context.
 */
const QUERY_TECHNICAL_SOLUTION_CONTEXT = {
